...
export default () => {
  ...
  const teamCapitain = club && club.atletas.find((item) => item.atleta_id === club.capitao_id);
  ...
  return (
    <SafeAreaViewContainer>
      <ScrollView
        ...
        refreshControl={<RefreshControl onRefresh={onRefetch} refreshing={isRefetchingClub} />}
              ...
              {!isMarketClose ? (
                <View className="flex-row justify-around items-center rounded-lg py-2">
                  <View className="justify-center items-center gap-1">
                    ...
                    <Text className="font-bold text-sm">{numberToString(club?.patrimonio)}</Text>

                    <Text
                      className={`text-sm font-semibold ${
                        club?.variacao_patrimonio > 0 ? 'text-green-500' : 'text-folly'
                      } `}>
                  ...
                  <View className="justify-center items-center gap-1">
                    ...
                    <Text className="font-bold text-sm">{numberToString(club?.pontos)}</Text>

                    <Text
                      className={`text-sm font-semibold ${
                        club?.variacao_pontos > 0 ? 'text-green-500' : 'text-folly'
                      } `}>
              ...
              <View className="p-2 rounded-lg">
                ...
                <View className="flex-row py-2 gap-x-1">
                  ...
                  <View className=" flex-1 flex-row items-center justify-between">
                    <View>
                      <Text className="text-sm font-semibold">{teamCapitain?.apelido}</Text>

                      <View className="flex-row items-center">
                        <Text className="text-xs font-light uppercase">
                          {(positions as IPositions)[teamCapitain?.posicao_id as number].nome}
                        </Text>
                      </View>
                    </View>
                      ...
                      <View className="items-center flex-row gap-x-2">
                        <View className="flex-row items-center">
                          <Text className="text-sm font-bold">
                            {numberToString(
                              playerStats.atletas[teamCapitain?.atleta_id]?.pontuacao
                            )}
                          </Text>
                        ...
                        <Text
                          className={`text-sm font-bold ${
                            playerStats?.atletas[teamCapitain?.atleta_id]?.pontuacao * 1.5 > 0
                              ? 'text-green-500'
                              : 'text-red-500'
                          }`}>
                          {numberToString(
                            playerStats?.atletas[teamCapitain?.atleta_id]?.pontuacao * 1.5
                          )}
                        </Text>
                      </View>
                    ) : (
