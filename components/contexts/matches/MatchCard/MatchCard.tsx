...
export function MatchCard({ match, homeClub, awayClub }: MatchCardProps) {
  ...
  return (
    <TouchableOpacity
      ...
      className="justify-center items-center bg-white p-2 mx-2 rounded-lg mb-1"
    >
      <View className="flex-row gap-x-1 justify-center items-center mb-3">
        ...
        <Text className="font-medium text-xs text-gray-500">{match.local}</Text>
      </View>
      ...
      {match.valida ? (
        ...
        >
          <Text className="text-xs">Esta partida é inválida para a rodada</Text>
        </View>
      )}
    </TouchableOpacity>
  );
